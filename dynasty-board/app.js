(function () {
  ...
  function render() {
    ...
    $("count").textContent = rows.length + " players · Superflex snapshot " + state.data.updated;
    ...
    $("body").innerHTML = rows.map(function (p) {
      return (
        '<tr class="' + (p.you ? "you" : "") + '">' +
          ...
          '<span class="meta"><span class="pos">' + p.pos + "</span> " + p.team + (p.you ? " · you" : "") + "</span></td>" +
  ...
  function bind() {
    ...
    $("headRow").addEventListener("click", function (e) {
      ...
      if (state.sort === key) state.dir = state.dir === "asc" ? "desc" : "asc";
        ...
        state.sort = key;
        state.dir = key === "value" || key === "name" ? "desc" : "asc";
      }
      render();
    });
  }

  fetch("rankings.json?v=1")
    .then(function (r) { return r.json(); })
    .then(function (data) {
      state.data = data;
      $("headerSub").textContent = "Superflex · " + data.updated;
      $("footnote").innerHTML =
        "Positional ranks. Avg skips blanks. Value is RotoWire Superflex JTV. Highlighted rows are on your roster. " +
        data.sources.map(function (s) {
          return "<strong>" + s.label + "</strong> — " + s.note;
        }).join(" · ") +
        ' · <a href="https://keeptradecut.com/dynasty-rankings">KTC</a> · ' +
        '<a href="https://www.rotowire.com/football/article/dynasty-trade-value-chart-september-2026-update-132022">RotoWire</a> · ' +
        '<a href="https://www.fantasycalc.com/dynasty-rankings">FantasyCalc</a> · ' +
        '<a href="https://www.fantasypros.com/nfl/rankings/nick-ercolano-bdge.php?position=OP&amp;type=dynasty">BDGE</a> · ' +
        '<a href="https://www.fantasypros.com/nfl/fantasy-football-rankings/dynasty-wr.php">FantasyPros</a>';
      renderChips();
      bind();
      render();
    })
